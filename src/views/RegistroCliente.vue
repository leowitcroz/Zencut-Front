    ...
    <div class="card border-0 shadow-sm p-4 position-relative">
        ...
        <form @submit.prevent="handleRegister">
            ...
            <div class="mb-3">
                ...
                <TelefoneInput v-model="form.telefone" required />
            </div>
...
onMounted(async () => {
    ...
    if (!isMainDomain && partes[0] !== 'adm' && partes[0] !== 'admin') {
        try {
            const res = await axios.get(`${API_URL}/tenants/info/${partes[0]}`);
            // Subdomínio da própria loja do dono da plataforma — não é uma barbearia
            // cliente de verdade, então não existe cadastro de cliente final aqui.
