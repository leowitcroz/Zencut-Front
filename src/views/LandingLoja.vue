<template>
    <div class="landing-loja">
        ...
        <template v-else>
            <!-- HEADER -->
            <header class="loja-header py-3" :style="{ background: loja.corPrimaria, color: corTexto }">
                <div class="container d-flex align-items-center justify-content-between">
                    <h3 class="fw-bold mb-0 text-truncate">{{ loja.nomeNegocio }}</h3>
                    <router-link to="/login" class="btn fw-bold px-3 flex-shrink-0"
                        :style="{ background: loja.corSecundaria, color: corTextoSecundaria, border: 'none' }">
            ...
            <section class="loja-hero text-white" :style="heroStyle">
                <div class="container loja-hero-content">
                    <div class="loja-hero-inner">
                        <h1 class="display-5 fw-bold mb-3 loja-hero-title">{{ loja.nomeNegocio }}</h1>
                        <p class="lead mb-0 loja-hero-title">{{ loja.descricaoLoja || 'Marque seu horário sem sair de casa.' }}</p>
                    </div>
                </div>
            </section>
            ...
            <section v-if="loja.servicos?.length" class="py-5" :class="{ 'bg-light': !secaoEhEscura('servicos') }"
                ...
                <div class="container py-3">
                    ...
                    <div class="servicos-carousel-wrap position-relative">
                        <button v-if="loja.servicos.length > 1" type="button" class="carousel-nav-btn start-0"
                            @click="rolarServicos(-1)" aria-label="Anterior">
                            <i class="bi bi-chevron-left"></i>
                        </button>
                        <div class="servicos-carousel" ref="servicosCarouselRef">
                            <div class="servico-card-carousel" v-for="s in loja.servicos" :key="s.nome">
                                <div class="card border-0 shadow-sm h-100 overflow-hidden">
                                    ...
                                    <div v-else class="servico-foto d-flex align-items-center justify-content-center"
                                        :style="{ background: loja.corPrimaria + '14' }">
                                        <i class="bi bi-scissors fs-1" :style="{ color: loja.corPrimaria }"></i>
                                    </div>
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-start mb-1">
                                            <h6 class="fw-bold mb-0">{{ s.nome }}</h6>
                                            <span class="fw-bold text-nowrap ms-2" :style="{ color: loja.corPrimaria }">{{ formatarValor(s.valor) }}</span>
                                        </div>
                        ...
                        <button v-if="loja.servicos.length > 1" type="button" class="carousel-nav-btn end-0"
                            @click="rolarServicos(1)" aria-label="Próximo">
                        ...
                        <div class="row align-items-center g-4">
                            <div class="col-md-5" :class="i % 2 === 0 ? 'order-md-1' : 'order-md-2'">
                                ...
                                <div v-else class="rounded-4 equipe-foto d-flex align-items-center justify-content-center w-100"
                                    :style="{ background: loja.corSecundaria + '22', color: loja.corSecundaria }">
            ...
            <section v-if="loja.planos?.length" class="py-5 text-center"
                ...
                <div class="container py-3">
                    ...
                    <div class="planos-carousel-wrap position-relative">
                        <button v-if="loja.planos.length > 3" type="button" class="carousel-nav-btn start-0"
                            @click="rolarPlanos(-1)" aria-label="Anterior">
                            <i class="bi bi-chevron-left"></i>
                        </button>
                        <div class="planos-carousel" ref="planosCarouselRef">
                            <div class="plano-card-carousel" v-for="p in loja.planos" :key="p.id">
                                <button type="button" class="plano-card text-start w-100 h-100"
                                    ...
                                    <div class="d-flex align-items-baseline gap-1 mb-2">
                                        <span class="fs-3 fw-bold" :style="{ color: loja.corSecundaria }">{{ formatarValor(p.valorMensal) }}</span>
                                    ...
                                    <span class="plano-cta" :style="{ color: loja.corSecundaria }">
                                        <i class="bi bi-whatsapp me-1"></i>Falar sobre esse plano
                                    </span>
                                </button>
                            </div>
                        </div>
                        <button v-if="loja.planos.length > 3" type="button" class="carousel-nav-btn end-0"
                            @click="rolarPlanos(1)" aria-label="Próximo">
                            <i class="bi bi-chevron-right"></i>
                        </button>
                    </div>
                </div>
                ...
                <div class="container py-3">
                    ...
                    <div class="contato-loja-box mx-auto">
                        <div class="contato-loja-photo">
                            <img v-if="loja.bannerUrl" :src="loja.bannerUrl" class="contato-loja-photo-img">
                            <div v-else class="contato-loja-photo-img d-flex align-items-center justify-content-center"
                                :style="{ background: loja.corPrimaria }">
                        ...
                        <div class="contato-loja-form-side" :style="estiloContatoForm">
                            <div class="w-100">
                                <span class="contato-loja-tag mb-3" :style="{ color: loja.corSecundaria, borderColor: loja.corSecundaria + '66', background: loja.corSecundaria + '1f' }">
                                ...
                                <form @submit.prevent="enviarContato">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea v-model="formContato.mensagem" class="form-control contato-loja-input" rows="3"
                                            placeholder="Quero agendar um horário..."></textarea>
                                    </div>
                                    <button class="btn btn-success w-100 py-2 fw-bold" :disabled="!linkWhatsapp">
                                ...
                                <div class="d-flex gap-3 mt-4">
                                    <span v-if="loja.endereco" class="small" style="opacity: .65;">
                                        <i class="bi bi-geo-alt-fill me-1"></i>{{ loja.endereco }}
                                    </span>
                                    <a v-if="linkInstagram" :href="linkInstagram" target="_blank" class="small" :style="{ color: corTexto, opacity: .65 }">
            ...
            <!-- FOOTER -->
            <footer class="py-4 text-center small" :style="{ background: loja.corPrimaria, color: corTexto }">
                <div class="container">
                    <span class="fw-semibold">{{ loja.nomeNegocio }}</span>
...
const heroStyle = computed(() => {
    if (loja.value?.bannerUrl) {
        return { backgroundImage: `url(${loja.value.bannerUrl})`, backgroundSize: 'cover', backgroundPosition: 'center' };
    }
    return { background: `linear-gradient(135deg, ${loja.value?.corPrimaria || '#0D1B2A'} 0%, ${loja.value?.corSecundaria || '#0D6EFD'} 130%)` };
});
...
const corTexto = computed(() => corContrastante(loja.value?.corPrimaria));
...
const linkWhatsapp = computed(() => {
    ...
    const msg = formContato.mensagem || `Olá! Vim pelo site da ${loja.value?.nomeNegocio || 'loja'} e quero agendar um horário.`;
    return `https://wa.me/55${numero}?text=${encodeURIComponent(msg + nome)}`;
});

const linkInstagram = computed(() => {
    const handle = (loja.value?.instagram || '').trim().replace(/^@/, '');
...
const rolarServicos = (direcao) => {
    servicosCarouselRef.value?.scrollBy({ left: direcao * 300, behavior: 'smooth' });
};

const planosCarouselRef = ref(null);
const rolarPlanos = (direcao) => {
    planosCarouselRef.value?.scrollBy({ left: direcao * 320, behavior: 'smooth' });
};

// Cliente ainda não consegue assinar sozinho (quem vincula é a própria loja) —
// então o clique no card chama direto no WhatsApp perguntando sobre o plano.
const falarSobrePlano = (plano) => {
    const numero = (loja.value?.whatsapp || '').replace(/\D/g, '');
    const msg = encodeURIComponent(`Olá! Vim pelo site da ${loja.value?.nomeNegocio || 'loja'} e quero saber mais sobre o plano "${plano.nome}".`);
    if (numero) {
...
.servicos-carousel {
    ...
    /* Centraliza quando os cards cabem todos na tela; quando não cabem, o
       overflow-x assume e ele rola normalmente a partir do início. */
