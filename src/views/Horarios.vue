                                    ...
                                    <div class="card border bg-light shadow-none">
                                        ...
                                        <div class="card-body">
                                            <div class="alert alert-warning border-0 py-2 small mb-3"
                                                v-if="dataInicio === getHoje()">
                                                <i class="bi bi-info-circle-fill me-1"></i> Horários que já passaram hoje
                                                ({{ horaAtualFormatada }}) ficam bloqueados e não podem ser abertos.
                                            </div>

                                            <div class="d-flex flex-wrap gap-2 mb-4">
                                                <button v-for="(hora, index) in horasDisponiveis" :key="index"
                                                    class="btn" :disabled="horaJaPassou(hora)"
                                                    :title="horaJaPassou(hora) ? 'Este horário já passou hoje' : ''"
                                                    :class="{ 'btn-outline-secondary bg-white': !horariosTemplate.includes(hora), 'btn-primary shadow-sm': horariosTemplate.includes(hora), 'opacity-50 text-decoration-line-through': horaJaPassou(hora) }"
...
const buscarBarbeirosDinamicos = async () => {
    try {
        ...
        barbeiros.value = response.data.filter(b => b.ativo && !ehContaAdmin(b.nome)).map(b => ({
...
const horaAtualFormatada = computed(() => getHoraAtual());

// Um horário só é considerado "passado" se a data em questão for HOJE e a hora já tiver virado
const horaJaPassou = (hora, dataReferencia = dataInicio.value) => {
    return dataReferencia === getHoje() && hora <= getHoraAtual();
};
...
const toggleHorarioTemplate = (h) => {
    if (horaJaPassou(h)) {
        showToast('Horário inválido', 'Este horário já passou hoje e não pode ser aberto.', 'warning');
        return;
    }
...
const adicionarIntervaloAFila = () => {
    ...
    let count = 0;
    let horariosIgnoradosPorSeremPassado = 0;

    while (curr <= end) {
        const key = curr.toISOString().split('T')[0];
        if (!filaHorarios.value[key]) filaHorarios.value[key] = [];

        // Se o dia processado for HOJE, nenhuma hora que já passou pode entrar na fila
        const horasValidasParaEsteDia = horariosTemplate.value.filter(h => !horaJaPassou(h, key));
        horariosIgnoradosPorSeremPassado += horariosTemplate.value.length - horasValidasParaEsteDia.length;

        horasValidasParaEsteDia.forEach(h => {
    ...
    const avisoPassado = horariosIgnoradosPorSeremPassado > 0
        ? ` (${horariosIgnoradosPorSeremPassado} horário(s) de hoje já passado(s) foram ignorados)`
        : '';
    showToast('Pronto', `${count} dia(s) processado(s)${avisoPassado}. Não se esqueça de Gravar no Banco.`, horariosIgnoradosPorSeremPassado > 0 ? 'warning' : 'success');
}
...
const limparFila = async () => {
    const ok = await confirmModalRef.value.confirmar('Esvaziar toda a fila de envio?', { titulo: 'Limpar fila', textoConfirmar: 'Esvaziar' });
    if (ok) filaHorarios.value = {};
}
...
const deletarHorario = async (h, evt) => {
    const ok = await confirmModalRef.value.confirmar(`Tem certeza que deseja excluir o horário das ${h.horario || h.horaInicio}?`, { titulo: 'Excluir horário', textoConfirmar: 'Excluir' });
    if (!ok) return;
